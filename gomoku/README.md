...
- `train/`: 单轮与持续训练脚本
- `eval/`: 新旧模型对战评估与晋升依据
- `training_monitor.py`: 训练状态监控工具

## 训练状态监控

```bash
python training_monitor.py              # 完整报告（回放池 + 训练 + 对局 + 预警）
python training_monitor.py --watch      # 仅预警检查（适合快速巡检）
python training_monitor.py --replay-only  # 仅回放池分析
python training_monitor.py --trend      # 仅训练趋势（loss + arena + 样本量）
python training_monitor.py --games      # 仅对局分析（胜负率 + 步数趋势）

# 筛选与分档
python training_monitor.py --games --last-games 2000 --bucket-size 100  # 最近2000局，100局一档
python training_monitor.py --watch --last-games 1000                    # 对最近1000局巡检
```
...
| 对局 | 平局率 | > 5% |
| 训练 | Loss下降速度 | > 0.01/轮 |
| 训练 | Arena WR | 不连续5轮=0.5 |

预警信号：
- **价值偏斜**：回放池中+1/-1标签比例失衡 → 可能是黑白偏斜
- **对局过短**：平均步数 < 15 → 模式坍塌
- **黑胜率失控**：黑方胜率 > 75% → 先手优势被放大
- **Loss停滞**：最近5轮 Loss 几乎不变 → 学习饱和
- **Arena停滞**：连续多轮 WR=0.50 → latest 无法超越 best
